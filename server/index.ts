...
import { registerChatRainRoutes } from './routes/chatRain.ts';
import { registerFriendRoutes } from './routes/friends.ts';
import { registerSystemRoutes } from './routes/system.ts';
import { registerTournamentRoutes } from './routes/tournaments.ts';
import { registerWalletPaymentRoutes } from './routes/walletPayments.ts';
...
const discordRedirectUri = (process.env.DISCORD_REDIRECT_URI || `${apiBaseUrl}/api/discord/connect/callback`).trim();
const siteAccessUsername = process.env.SITE_ACCESS_USERNAME || 'pasus';
const siteAccessPassword = process.env.SITE_ACCESS_PASSWORD || 'launch';
...
app.post('/api/activity/bets', requireAuth, async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    if (blockedClientGames.has(gameKey)) {
      return res.status(403).json({ error: 'This game is temporarily disabled until server-authoritative logic is implemented.' });
    }
