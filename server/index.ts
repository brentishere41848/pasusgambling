...
import {
  CASE_BATTLE_CASES,
  CASE_BATTLE_FORMATS,
  CASE_BATTLE_REVEAL_MS,
  CaseBattleFormatKey,
  createBattlePublicSeed,
  generateCaseBattleServerSeed,
  getBattleVisibleRounds,
  getBotProfile,
  getCaseBattleFormat,
  getCaseBattleTeamLayout,
  getCaseDefinition,
  getCaseSelectionPrice,
  hashCaseBattleServerSeed,
  normalizeCaseBattleFormat,
  normalizeCaseSelection,
  resolveBattleDrop,
  resolveCaseBattleState,
} from './caseBattles.js';
...
function mapCaseBattlePlayerRow(row: any): CaseBattlePlayerRow {
  return {
    id: Number(row.id),
...
function mapCaseBattleDropRow(row: any): CaseBattleDropRow {
  return {
    id: Number(row.id),
    battle_id: Number(row.battle_id),
    ...
    item_image: String(row.item_image || ''),
    item_value: Number(row.item_value || 0),
    rarity: String(row.rarity || 'consumer'),
...
function getCaseBattleModeLabel(formatKey: CaseBattleFormatKey) {
  return getCaseBattleFormat(formatKey).label;
}

function getOpenCaseBattleSlotIndexes(players: CaseBattlePlayerRow[], totalSlots: number) {
  const occupied = new Set(players.map((player) => player.slot_index));
...
async function fetchCaseBattleRows(client: Pool | PoolClient, battleId: number) {
  const [battleResult, playerResult, dropResult] = await Promise.all([
    client.query(
      `SELECT *
       FROM case_battles
       WHERE id = $1
    ...
    client.query(
      `SELECT *
       FROM case_battle_round_drops
       WHERE battle_id = $1
       ORDER BY round_index ASC, player_id ASC`,
      [battleId]
    ),
  ]);
  ...
  return {
    ...
    players: playerResult.rows.map(mapCaseBattlePlayerRow),
    drops: dropResult.rows.map(mapCaseBattleDropRow),
  };
}

function buildCaseBattlePayload(battleRow: any, playerRows: CaseBattlePlayerRow[], dropRows: CaseBattleDropRow[]) {
  if (!battleRow) {
    return null;
  }

  const formatKey = normalizeCaseBattleFormat(battleRow.format_key) || '1v1';
  const totalSlots = Number(battleRow.total_slots || getCaseBattleFormat(formatKey).slots);
  const caseIds = Array.isArray(battleRow.case_ids) ? battleRow.case_ids.map((entry: unknown) => String(entry || '')) : [];
  const caseDefinitions = caseIds
    .map((caseId) => getCaseDefinition(caseId))
    .filter((entry): entry is NonNullable<typeof entry> => Boolean(entry));
  const visibleRounds = battleRow.status === 'completed' ? caseIds.length : Math.min(caseIds.length, getBattleVisibleRounds(battleRow.started_at));
  const players = playerRows.map((player) => {
    const drops = dropRows
      .filter((drop) => drop.player_id === player.id)
      .map((drop) => ({
        roundIndex: drop.round_index,
        caseId: drop.case_id,
        itemId: drop.item_id,
        itemName: drop.item_name,
        itemImage: drop.item_image,
        itemValue: drop.item_value,
        rarity: drop.rarity as 'consumer' | 'industrial' | 'mil-spec' | 'restricted' | 'classified' | 'covert',
      }));
    const totalValue = drops.reduce((sum, drop) => sum + drop.itemValue, 0);
    const visibleValue = drops.filter((drop) => drop.roundIndex < visibleRounds).reduce((sum, drop) => sum + drop.itemValue, 0);
  ...
  const resolved = resolveCaseBattleState(formatKey, players, caseIds.length);
  const openSlots = getOpenCaseBattleSlotIndexes(playerRows, totalSlots);

  return {
    battle: {
      id: Number(battleRow.id),
      formatKey,
      modeLabel: getCaseBattleModeLabel(formatKey),
      status: String(battleRow.status || 'waiting'),
      freeToJoin: Boolean(battleRow.free_to_join),
      totalSlots,
      openSlots,
      creatorUserId: Number(battleRow.created_by_user_id),
      seatCost: Number(battleRow.case_price_total || 0),
      totalPot: Number(battleRow.total_pot || 0),
      createdAt: battleRow.created_at,
      startedAt: battleRow.started_at,
      completedAt: battleRow.completed_at,
      revealMs: CASE_BATTLE_REVEAL_MS,
      serverSeedHash: String(battleRow.server_seed_hash || ''),
      publicSeed: String(battleRow.public_seed || ''),
      serverSeed: battleRow.status === 'completed' ? String(battleRow.server_seed || '') : null,
      winningTeamIndexes: resolved.winningTeamIndexes,
      winningScore: resolved.winningScore,
      standings: resolved.standings,
      cases: caseDefinitions.map((entry) => ({
        id: entry.id,
        name: entry.name,
        image: entry.image,
        price: entry.price,
...
async function finalizeCaseBattleIfReady(client: Pool | PoolClient, battleId: number) {
  ...
  const completedAt = battle.completed_at ? new Date(battle.completed_at).getTime() : 0;
  const payoutAppliedAt = battle.payout_applied_at ? new Date(battle.payout_applied_at).getTime() : 0;
  ...
  if (!payoutAppliedAt) {
    ...
    const players = playerResult.rows.map(mapCaseBattlePlayerRow);
    const drops = dropResult.rows.map(mapCaseBattleDropRow);
    const caseIds = Array.isArray(battle.case_ids) ? battle.case_ids.map((entry: unknown) => String(entry || '')) : [];
    const formatKey = normalizeCaseBattleFormat(battle.format_key) || '1v1';
    const resolved = resolveCaseBattleState(
      formatKey,
      players.map((player) => ({
        playerId: player.id,
        userId: player.user_id,
        ...
        teamIndex: player.team_index,
        paidAmount: player.paid_amount,
        totalValue: drops.filter((drop) => drop.player_id === player.id).reduce((sum, drop) => sum + drop.item_value, 0),
        visibleValue: 0,
        drops: [],
      })),
      caseIds.length
    ...
    const winners = players.filter(
      (player) => !player.is_bot && player.user_id && resolved.winningTeamIndexes.includes(player.team_index)
    );
    const splitPayout = winners.length > 0 ? Math.floor(Number(battle.total_pot || 0) / winners.length) : 0;

    for (const winner of winners) {
      await ensureWallet(client, Number(winner.user_id));
      await client.query(
        `UPDATE wallets
         SET balance = balance + $1,
             updated_at = NOW()
         WHERE user_id = $2`,
        [splitPayout, winner.user_id]
      );

      await client.query(
        `INSERT INTO bet_activities (user_id, game_key, wager, payout, multiplier, outcome, detail)
         VALUES ($1, 'case-battles', $2, $3, $4, 'win', $5)`,
        [
          winner.user_id,
          Number(winner.paid_amount || 0),
          splitPayout,
          Number(battle.total_pot || 0) > 0 ? Number((splitPayout / Math.max(1, Number(winner.paid_amount || 1))).toFixed(4)) : 0,
          `Won ${getCaseBattleModeLabel(formatKey)} case battle #${battleId}`,
        ]
      );
    }
    ...
    for (const loser of losers) {
      await client.query(
        `INSERT INTO bet_activities (user_id, game_key, wager, payout, multiplier, outcome, detail)
         VALUES ($1, 'case-battles', $2, 0, 0, 'loss', $3)`,
        [
          loser.user_id,
          Number(loser.paid_amount || 0),
          `Lost ${getCaseBattleModeLabel(formatKey)} case battle #${battleId}`,
        ]
      );
    }
...
async function tryStartCaseBattle(client: Pool | PoolClient, battleId: number) {
  const battleResult = await client.query(
    `SELECT *
     FROM case_battles
     WHERE id = $1
  ...
  const players = playerResult.rows.map(mapCaseBattlePlayerRow);
  const totalSlots = Number(battle.total_slots || 0);
  if (players.length !== totalSlots) {
    return battle;
  }

  const caseIds = Array.isArray(battle.case_ids) ? battle.case_ids.map((entry: unknown) => String(entry || '')) : [];
  const serverSeed = String(battle.server_seed || '');
  const publicSeed = String(battle.public_seed || '');

  for (const player of players) {
    for (let roundIndex = 0; roundIndex < caseIds.length; roundIndex += 1) {
      const caseId = caseIds[roundIndex];
      const item = resolveBattleDrop(serverSeed, publicSeed, battleId, player.slot_index, roundIndex, caseId);

      await client.query(
        `INSERT INTO case_battle_round_drops (
          battle_id, player_id, round_index, case_id, item_id, item_name, item_image, item_value, rarity
        )
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
        [battleId, player.id, roundIndex, caseId, item.id, item.name, item.image, item.value, item.rarity]
      );
    }
  }

  const startedAt = new Date();
  const completedAt = new Date(startedAt.getTime() + caseIds.length * CASE_BATTLE_REVEAL_MS);
  await client.query(
    `UPDATE case_battles
     SET status = 'rolling',
         started_at = $2,
         ...
         updated_at = NOW()
     WHERE id = $1`,
    [battleId, startedAt.toISOString(), completedAt.toISOString()]
  );
...
async function initDb() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS case_battles (
      ...
      free_to_join BOOLEAN NOT NULL DEFAULT false,
      total_slots INT NOT NULL,
      case_ids JSONB NOT NULL,
      case_price_total BIGINT NOT NULL,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS case_battle_players (
      id BIGSERIAL PRIMARY KEY,
      battle_id BIGINT NOT NULL REFERENCES case_battles(id) ON DELETE CASCADE,
      user_id BIGINT REFERENCES users(id) ON DELETE SET NULL,
      username TEXT NOT NULL,
      avatar_url TEXT,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS case_battle_round_drops (
      id BIGSERIAL PRIMARY KEY,
      battle_id BIGINT NOT NULL REFERENCES case_battles(id) ON DELETE CASCADE,
      player_id BIGINT NOT NULL REFERENCES case_battle_players(id) ON DELETE CASCADE,
...
app.get('/api/case-battles/config', (_req, res) => {
  return res.json({
    formats: CASE_BATTLE_FORMATS,
    cases: CASE_BATTLE_CASES.map((entry) => ({
      id: entry.id,
      name: entry.name,
      price: entry.price,
      image: entry.image,
...
app.get('/api/case-battles', async (_req, res) => {
  ...
  try {
    ...
    for (const row of result.rows) {
      await client.query('BEGIN');
      const battle = await finalizeCaseBattleIfReady(client, Number(row.id));
      const snapshot = await fetchCaseBattleRows(client, Number(row.id));
      await client.query('COMMIT');
      const payload = buildCaseBattlePayload(battle || snapshot.battle, snapshot.players, snapshot.drops);
      if (payload) {
    ...
    await client.query('ROLLBACK');
    console.error(error);
    return res.status(500).json({ error: 'Failed to load case battles.' });
  } finally {
    client.release();
  }
});

app.get('/api/case-battles/:id', async (req, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    await client.query('BEGIN');
    const battle = await finalizeCaseBattleIfReady(client, battleId);
    const snapshot = await fetchCaseBattleRows(client, battleId);
    await client.query('COMMIT');
    ...
    return res.json({ battle: buildCaseBattlePayload(battle || snapshot.battle, snapshot.players, snapshot.drops) });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error(error);
...
app.post('/api/case-battles', requireAuth, async (req: AuthedRequest, res) => {
  const client = await pool.connect();

  try {
    const formatKey = normalizeCaseBattleFormat(req.body.formatKey);
    const caseIds = normalizeCaseSelection(req.body.caseIds);
    const botCount = Math.max(0, Math.min(5, Number(req.body.botCount || 0)));
    const freeToJoin = normalizeBoolean(req.body.freeToJoin);

    if (!formatKey) {
      return res.status(400).json({ error: 'A valid battle format is required.' });
    }

    if (caseIds.length === 0) {
      return res.status(400).json({ error: 'Pick at least one case.' });
    }

    const format = getCaseBattleFormat(formatKey);
    if (botCount > format.slots - 1) {
      return res.status(400).json({ error: 'Too many bots for this battle format.' });
    }

    const seatCost = getCaseSelectionPrice(caseIds);
    const upfrontSeats = freeToJoin ? format.slots : 1 + botCount;
    const upfrontCost = seatCost * upfrontSeats;
    const teamLayout = getCaseBattleTeamLayout(formatKey);
    ...
    const walletResult = await client.query(
      `UPDATE wallets
       ...
       WHERE user_id = $2
         AND balance >= $1
       RETURNING balance, total_deposited, total_withdrawn`,
      [upfrontCost, req.auth!.user.id]
    );

    if (!walletResult.rowCount) {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: 'Insufficient balance for that battle.' });
    }

    const serverSeed = generateCaseBattleServerSeed();
    const insertBattle = await client.query(
      `INSERT INTO case_battles (
        created_by_user_id, format_key, status, free_to_join, total_slots, case_ids, case_price_total,
        total_pot, public_seed, server_seed, server_seed_hash
      )
      VALUES ($1, $2, 'waiting', $3, $4, $5::jsonb, $6, $7, $8, $9, $10)
      RETURNING *`,
      [
        req.auth!.user.id,
        formatKey,
        ...
        seatCost,
        seatCost * format.slots,
        createBattlePublicSeed(),
        serverSeed,
        hashCaseBattleServerSeed(serverSeed),
      ]
    );

    const battleId = Number(insertBattle.rows[0].id);
    await client.query(
      `INSERT INTO case_battle_players (
        battle_id, user_id, username, avatar_url, is_bot, slot_index, team_index, paid_amount
      )
      VALUES ($1, $2, $3, $4, false, 0, $5, $6)`,
      [battleId, req.auth!.user.id, req.auth!.user.username, avatarUrl, teamLayout[0], freeToJoin ? seatCost : seatCost]
    );

    for (let botIndex = 0; botIndex < botCount; botIndex += 1) {
      const slotIndex = botIndex + 1;
      const profile = getBotProfile(battleId + botIndex);
      await client.query(
        `INSERT INTO case_battle_players (
          battle_id, user_id, username, avatar_url, is_bot, slot_index, team_index, paid_amount
        )
        VALUES ($1, NULL, $2, $3, true, $4, $5, $6)`,
        [battleId, profile.username, profile.avatarUrl, slotIndex, teamLayout[slotIndex], freeToJoin ? 0 : seatCost]
      );
    }

    await tryStartCaseBattle(client, battleId);
    const snapshot = await fetchCaseBattleRows(client, battleId);
    await client.query('COMMIT');

    return res.status(201).json({
      battle: buildCaseBattlePayload(snapshot.battle, snapshot.players, snapshot.drops),
      wallet: sanitizeWallet(walletResult.rows[0]),
    });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error(error);
    return res.status(500).json({ error: 'Failed to create case battle.' });
  } finally {
    client.release();
  }
});

app.post('/api/case-battles/:id/join', requireAuth, async (req: AuthedRequest, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    if (battle.status !== 'waiting') {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: 'This battle already started.' });
    }
    ...
    const players = playersResult.rows.map(mapCaseBattlePlayerRow);
    if (players.some((player) => player.user_id === req.auth!.user.id)) {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: 'You are already in this battle.' });
    }

    const openSlots = getOpenCaseBattleSlotIndexes(players, Number(battle.total_slots || 0));
    if (openSlots.length === 0) {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: 'That battle is already full.' });
    }

    const formatKey = normalizeCaseBattleFormat(battle.format_key) || '1v1';
    const teamLayout = getCaseBattleTeamLayout(formatKey);
    const slotIndex = openSlots[0];
    const seatCost = Number(battle.case_price_total || 0);

    let walletRow: any = null;
    if (!battle.free_to_join) {
      await ensureWallet(client, req.auth!.user.id);
      const walletResult = await client.query(
        `UPDATE wallets
         SET balance = balance - $1,
         ...
         WHERE user_id = $2
           AND balance >= $1
         RETURNING balance, total_deposited, total_withdrawn`,
        [seatCost, req.auth!.user.id]
      );

      if (!walletResult.rowCount) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'Insufficient balance to join this battle.' });
      }
    ...
    await client.query(
      ...
      [battleId, req.auth!.user.id, req.auth!.user.username, avatarUrl, slotIndex, teamLayout[slotIndex], battle.free_to_join ? 0 : seatCost]
    );
...
app.post('/api/case-battles/:id/bots', requireAuth, async (req: AuthedRequest, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const battle = battleResult.rows[0];
    if (Number(battle.created_by_user_id) !== req.auth!.user.id) {
      await client.query('ROLLBACK');
      return res.status(403).json({ error: 'Only the creator can add bots.' });
    }

    if (battle.status !== 'waiting') {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: 'The battle has already started.' });
    }
    ...
    const players = playersResult.rows.map(mapCaseBattlePlayerRow);
    const openSlots = getOpenCaseBattleSlotIndexes(players, Number(battle.total_slots || 0));
    const botsToAdd = openSlots.slice(0, count);

    if (botsToAdd.length === 0) {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: 'There are no open slots left.' });
    }
      ...
      const walletResult = await client.query(
        `UPDATE wallets
         ...
         WHERE user_id = $2
           AND balance >= $1
         RETURNING balance, total_deposited, total_withdrawn`,
        [seatCost * botsToAdd.length, req.auth!.user.id]
      );

      if (!walletResult.rowCount) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'Insufficient balance to add bots.' });
      }
      walletRow = walletResult.rows[0];
    } else {
      const walletResult = await client.query(
    ...
    for (let index = 0; index < botsToAdd.length; index += 1) {
      const slotIndex = botsToAdd[index];
      const profile = getBotProfile(battleId + slotIndex + index);
      await client.query(
        `INSERT INTO case_battle_players (
          battle_id, user_id, username, avatar_url, is_bot, slot_index, team_index, paid_amount
        )
        VALUES ($1, NULL, $2, $3, true, $4, $5, $6)`,
        [battleId, profile.username, profile.avatarUrl, slotIndex, teamLayout[slotIndex], battle.free_to_join ? 0 : seatCost]
      );
    }
